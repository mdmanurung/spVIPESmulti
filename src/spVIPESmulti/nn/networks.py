    ...
    **kwargs
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if use_low_rank_mixer:
            # Lightweight rank-r bottleneck: (n_in → r → n_output)
            n_in_cat = n_input_shared + n_input_private
            self.mix_down = nn.Linear(n_in_cat, low_rank_mixer_rank)
            self.mix_up = nn.Linear(low_rank_mixer_rank, n_output)
        else:
