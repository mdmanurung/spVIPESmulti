...
1. The [contributor guide](contributing.md#contributing-guide). It contains information relevant to all developers who want to make a contribution.
...
:::{note}
If you are looking at this document in the [cookiecutter-scverse-instance] repository documentation, throughout this document the name of the project is `cookiecutter-scverse-instance`. Otherwise it should be replaced by your new project name: `spVIPESmulti`.
:::

Now that your new project repository has been created on github at `https://github.com/mdmanurung/spVIPESmulti` you can push your first commit to github.
To do this, simply follow the instructions on your github repository page or a more verbose walkthrough here:

Assuming you are in `/your/path/to/spVIPESmulti`. Add all files and commit.
...
git remote add origin https://github.com/mdmanurung/spVIPESmulti.git
...
Your project should be now available at `https://github.com/mdmanurung/spVIPESmulti`. While the repository at this point can be directly used, there are few remaining steps that needs to be done in order to achieve full functionality.
...
The workflows should now succeed and you will be able to find the code coverage at this link: `https://app.codecov.io/gh/mdmanurung/spVIPESmulti`. You might have to wait couple of minutes and the coverage of this repository should be ~60%.
...
1. Go to _Settings_ and copy **only** the token `_______-____-...`.

1. Go to _Settings_ of your newly created repository on GitHub.

1. Go to _Security > Secrets > Actions_.

1. Create new repository secret with name `CODECOV_TOKEN` and paste the token generated by codecov.

1. Past these additional lines in `/.github/workflows.test.yaml` under the **Upload coverage** step:
...
1. Go back to github `Actions` page an re-run previously failed jobs.
...
If your project is private, there are ways to enable docs rendering on [readthedocs.org] but it is more cumbersome and requires a different subscription for read the docs. See the [RTD private repository import guide](https://docs.readthedocs.io/en/stable/guides/importing-private-repositories.html).
