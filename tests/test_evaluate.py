...
def _make_adata(n_cells: int = 8, n_groups: int = 2, n_dims: int = 4):
    ...
    adata.obs["groups"] = pd.Categorical(
        [group_names[i // per_group] for i in range(n_cells)]
    )
    adata.obs["cell_type"] = pd.Categorical(
        ["T" if i % 2 == 0 else "B" for i in range(n_cells)]
    )
...
class TestEvaluateReturnSchema:
    def test_keys_present(self):
        ...
        out = _ModelClass.evaluate(
            model, adata=adata, z_shared_key="X_spvm_shared", k=3
        )
    ...
    def test_metadata_fields_present(self):
        ...
        md = out["metadata"]
        for key in ("n_cells", "n_groups", "k", "label_key", "leiden_resolution",
                    "include_private", "used_precomputed_embedding"):
            assert key in md
...
class TestEvaluateEmbeddingFallback:
    def test_warns_when_precomputed_key_missing(self):
        ...
        model = _make_model(adata)
        out = _ModelClass.evaluate(
            model, adata=adata, z_shared_key="X_does_not_exist", k=3
        )
...
class TestEvaluateLabelHandling:
    def test_label_key_populates_metadata(self):
        ...
        model = _make_model(adata)
        out = _ModelClass.evaluate(
            model, adata=adata, z_shared_key="X_spvm_shared",
            label_key="cell_type", k=3
        )
...
class TestEvaluateHeldOutMetrics:
    def test_reads_latest_validation_metrics_from_history(self):
        ...
        model.history_ = {
            ...
            "reconstruction_loss_validation": pd.DataFrame(
                {"reconstruction_loss_validation": [10.0, 7.25]}
            ),
