...
-   Small-dataset integration metrics now clamp the requested kNN neighbourhood
    size to the available number of cells instead of failing inside
    `NearestNeighbors`.
-   `model.evaluate(...)` now reports label-dependent diagnostics as `NaN`
    when no valid `label_key` is supplied, matching the documented API.
-   `score_cells_on_factor(...)` now rejects negative latent-dimension indices.
-   The legacy Python-package CI workflow now installs the package before
    running tests, and the docs config tolerates older supported Sphinx versions.

## [1.0.1] — 2026-05-13

### Fixed

-   `spVIPESmulti.dataloaders.__all__` now lists only importable dataloader
    exports.
-   `spVIPESmulti.get_latent_representation(...)` now honors `indices` and
    uses the validated `AnnData` manager when a compatible `adata` argument is
    provided.
-   Single-modal inference now clamps observed library sizes before taking
    logs, avoiding `-inf` values for all-zero cells.
-   Negative-binomial reconstruction losses now evaluate likelihoods on raw
    count targets while keeping log-transformed inputs limited to encoder
    preprocessing.

### Changed

-   `get_latent_representation(...)` now documents its dictionary return
    schema, including per-group ordering semantics.
-   Declared `scvi-tools` compatibility now starts at `>=1.2.2.post2`; older
    1.x releases fail import under current dependency resolution before
    spVIPESmulti code is reached.
...
-   **Disentanglement objective** (inspired by CellDISECT's cross-covariate
    ...
    **Note on naming:** these are _not_ the original Mutual Information Gap
